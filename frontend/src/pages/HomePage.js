...
const SpecialCompCard = ({ c }) => {
    ...
    const typeBadge = getCompetitionTypeBadge(c);
            ...
            >
                ...
                <div className="relative p-6">
                    ...
                    <div className="flex items-center gap-2 mb-4 flex-wrap">
                        ...
                        <span className={`px-2 py-1 ${typeBadge.bgColor}/20 ${typeBadge.textColor === 'text-black' ? 'text-yellow-400' : typeBadge.textColor.replace('text-', 'text-').replace('white', 'gray-200')} text-xs font-bold rounded border ${typeBadge.bgColor.replace('bg-', 'border-')}/30`}>
                            {typeBadge.label}
                        </span>
                    </div>
            ...
            <div className="relative h-full flex flex-col">
                <div className="relative flex-1 min-h-[250px]">
                    ...
                    <div className="absolute top-3 left-3 flex gap-2">
                        ...
                        <span className={`px-2 py-1 ${typeBadge.bgColor} ${typeBadge.textColor} text-xs font-bold rounded`}>{typeBadge.label}</span>
                    </div>
                </div>
                <div className="p-5">
                    <h2 className="text-xl font-black text-white mb-3">{c.title}</h2>
...
// Helper function to get competition type badge
const getCompetitionTypeBadge = (c) => {
    // Check competition_type field
    if (c.competition_type === 'instant_win' || c.competition_type === 'instant') {
        return { label: 'INSTANT', bgColor: 'bg-yellow-500', textColor: 'text-black' };
    } else if (c.competition_type === 'classic') {
        return { label: 'CLASIC', bgColor: 'bg-blue-600', textColor: 'text-white' };
    } else if (c.competition_type === 'autodraw') {
        return { label: 'AUTODRAW', bgColor: 'bg-green-500', textColor: 'text-white' };
    }
    // Default to autodraw if not specified
    return { label: 'AUTODRAW', bgColor: 'bg-green-500', textColor: 'text-white' };
};
        ...
        <Link to={`/competitions/${c.competition_id}`} className="group block bg-[#12111a] rounded-xl overflow-hidden border border-white/5 hover:border-violet-500/30 transition-all">
            <div className="relative aspect-[4/3]">
                ...
                <div className="absolute top-2 left-2">
                    <span className={`px-2 py-0.5 ${typeBadge.bgColor} ${typeBadge.textColor} text-[10px] font-bold rounded`}>{typeBadge.label}</span>
                </div>
