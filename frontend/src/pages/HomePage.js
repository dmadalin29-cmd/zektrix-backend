...
import { ArrowRight, Zap, ChevronRight, MessageCircle, Radio, Users, Trophy, Ticket, Sparkles, Star, Crown } from 'lucide-react';
                ...
                <div className="relative p-6">
                    ...
                    <div className="mb-4">
                        <div className="flex justify-between text-sm mb-1.5">
                            <span className="text-[#A39EBD]">Progres: <span className="text-white font-semibold">{progress.toFixed(1)}%</span></span>
                            <span className="text-[#A39EBD]">Libere: <span className="text-emerald-400 font-semibold">{remaining.toLocaleString()}</span></span>
                        </div>
                        <div className="h-2.5 bg-white/[0.06] rounded-full overflow-hidden">
                            <div className="h-full bg-gradient-to-r from-amber-500 via-orange-500 to-red-500 rounded-full transition-all duration-500 relative" style={{width: `${Math.max(progress, 2)}%`}}>
                    ...
                    <div className="p-5">
                        ...
                        <div className="mb-3">
                            <div className="flex justify-between text-xs mb-1.5">
                                <span className="text-[#A39EBD]">Progres: <span className="text-white font-semibold">{progress.toFixed(1)}%</span></span>
                                <span className="text-[#A39EBD]">Libere: <span className="text-violet-400 font-semibold">{remaining.toLocaleString()}</span></span>
                            </div>
                            <div className="h-2 bg-white/[0.06] rounded-full overflow-hidden">
                                <div className="h-full bg-gradient-to-r from-violet-500 to-[#FF5E00] rounded-full transition-all" style={{width: `${Math.max(progress, 2)}%`}} />
                            </div>
                        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.05 * (index || 0) }}>
            ...
            >
                <div className="relative aspect-[4/3] overflow-hidden">
                    ...
                    {c.competition_type === 'instant_win' && (
                        <div className="absolute top-2.5 left-2.5"><span className="px-2 py-1 bg-emerald-500/80 backdrop-blur-sm text-white text-[10px] font-bold rounded-full">AUTODRAW</span></div>
                    )}
                    {c.competition_type === 'draw' && (
                        <div className="absolute top-2.5 left-2.5"><span className="px-2 py-1 bg-violet-500/80 backdrop-blur-sm text-white text-[10px] font-bold rounded-full">DRAW</span></div>
                    )}
                ...
                <div className="p-4">
                    <h3 className="font-bold text-white text-sm mb-2.5 truncate tracking-tight">{c.title}</h3>
                    <div>
                        <div className="flex justify-between text-[10px] mb-1">
                            <span className="text-[#6E6987]">{progress.toFixed(0)}%</span>
                            <span className="text-[#6E6987]">{remaining.toLocaleString()} libere</span>
                        </div>
                        <div className="h-1.5 bg-white/[0.06] rounded-full overflow-hidden">
                            <div className="h-full bg-gradient-to-r from-violet-500 to-[#FF5E00] rounded-full" style={{width: `${Math.max(progress, 3)}%`}} />
                        </div>
                    </div>
                </div>
            </Link>
