...
async def send_web_push(subscription: dict, data: dict) -> bool:
    ...
    # Ensure tag for mobile grouping
    if "tag" not in data:
        data["tag"] = f"zektrix-{data.get('type', 'general')}"
    ...
    try:
        ...
        logger.error(f"Push failed ({status_code}): {str(e)[:200]}")
...
async def notify_user_push(db, user_id: str, title: str, body: str, url: str = "https://zektrix.uk"):
    ...
    for sub in subs:
        try:
            await send_web_push(sub, {"title": title, "body": body, "url": url, "type": "user"})
            sent += 1
        except Exception as e:
            logger.warning(f"Push to user {user_id} device failed: {e}")
            if "410" in str(e) or "404" in str(e):
                await db.push_subscriptions.delete_one({"endpoint": sub["endpoint"]})
    if sent > 0:
        logger.info(f"Push sent to user {user_id}: {sent}/{len(subs)} devices")
...
async def notify_competition_participants_push(db, competition_id: str, title: str, body: str, url: str = None):
    ...
    subs = await db.push_subscriptions.find({"user_id": {"$in": user_ids}}, {"_id": 0}).to_list(5000)
    sent = 0
    for sub in subs:
        try:
            await send_web_push(sub, {"title": title, "body": body, "url": url, "type": "competition"})
            sent += 1
        except Exception as e:
            if "410" in str(e) or "404" in str(e):
                await db.push_subscriptions.delete_one({"endpoint": sub["endpoint"]})
    logger.info(f"Competition push sent: {sent}/{len(subs)} devices for {competition_id}")
...
async def notify_admins_push(db, title: str, body: str, url: str = "https://zektrix.uk/admin"):
    ...
    subs = await db.push_subscriptions.find({"user_id": {"$in": admin_ids}}, {"_id": 0}).to_list(50)
    for sub in subs:
        try:
            await send_web_push(sub, {"title": title, "body": body, "url": url, "type": "admin"})
        except Exception as e:
            if "410" in str(e) or "404" in str(e):
                await db.push_subscriptions.delete_one({"endpoint": sub["endpoint"]})
